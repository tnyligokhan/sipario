...
<section class="blm kagit2" x-data="sssArama">
    ...
    <div class="kap">
        ...
        <x-site.pano ince class="sss-bos" x-show="bulunan() === 0" x-cloak>
            ...
            {{-- Cümlenin tamamı TELEFONA bağlı: "yazmayın, arayın — …".
                 ⚠️ Eskiden `$sw['kanal'][0]` yazıyordu, yani "listenin ilk kanalı" — bu, telefon
                 kanalı süzülüp düştüğü an sıfırıncı sırayı E-POSTAYA bırakıyordu ve cümle
                 "yazmayın, arayın — destek@sipario.com.tr" hâline geliyordu (2026-09-01'de WhatsApp
                 satırı kalkınca görünür oldu). Artık kanal TÜRÜNE bakılıyor, sırasına değil. --}}
            @php($telefon = collect($sw['kanal'])->firstWhere('ik', 'telefon'))
            @if ($telefon && $telefon['href'])
                <p class="gvd">Aradığınızı bulamadıysanız yazmayın, arayın — {{ $telefon['deger'] }}.</p>
            @endif
