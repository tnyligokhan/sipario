...
<div class="hb">
    {{--
        ── "OTO-SIRALAMA HAKKI" DİLİ SADELEŞTİ (2026-08-19) ────────────────────────────────
        Bu ekranın tamamı iki bilinmeyen kelimenin üstüne kuruluydu: "oto-sıralama" bizim iç
        adımız, "hak" ise ne olduğu belirsiz bir birim. Bayi buraya bir şey SATIN ALMAK için
        geliyor ve ne aldığını okuyamıyordu.

        Yapılan iş bir "kelime değiştirme" değil: sayılan şeyin ne olduğu (bir sıralama işlemi),
        ne zaman harcandığı (uygulama sıraladığında) ve ne zaman harcanmadığı (elle
        sürüklerken) düz cümlelerle yazıldı. Rakam ve mekanik aynı — yalnız okunabilir oldu.
    --}}
    <x-site.pano etiket="{{ $deneme ? 'Denemede kurye yolu sıralama' : 'Kurye yolu sıralama' }}">
        <x-site.kota etiket="Kullanılan" :kullanilan="$hak['kullanilan']" :toplam="$hak['toplam']"
            alt="{{ $hak['kalan'] }} sıralama kaldı · {{ $deneme ? 'deneme boyunca geçerli' : 'her ay '.$bayi->route_credits_monthly.' tane yenilenir' }}" />
        <hr class="ayrac">
        <p class="gvd">Kuryenin gideceği adresleri uygulamaya sıralattığınızda bir tane düşer. Sırayı parmağınızla kendiniz dizerseniz hiçbir şey harcanmaz. Satın aldığınız ek paketlerin süresi dolmaz, sonraki aya devreder.</p>
    </x-site.pano>

    <x-site.pano etiket="Ek sıralama paketi" :ic="false">
        @if ($deneme)
            <div style="padding:22px 22px 0">
                <x-site.kutu tur="mor" ikon="bilgi">
                    Denemede {{ $hak['toplam'] }} sıralama zaten yüklü geliyor. Ek paketleri aboneliği başlattıktan sonra satın alabilirsiniz.
                </x-site.kutu>
            </div>
        @endif
        ...
        @if ($this->hakPaketleri->isEmpty())
            <div class="hb-bos">
                ...
                <p class="kucuk">Daha fazlasına ihtiyacınız varsa destek hattından bize ulaşın.</p>
            </div>
        @else
            <div class="hak-grid hb-hak">
                @foreach ($this->hakPaketleri as $paket)
                    <div class="hak">
                        ...
                        <span class="kucuk">tanesi {{ $this->tlk($paket->price_kurus / max(1, $paket->quantity)) }}</span>
                        @if ($deneme)
