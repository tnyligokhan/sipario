...
1. **`dev` → `main` birleştirmesi.** `main` şu an `d4bb9f6`, `dev`'den 2 commit geride ve bu vardiyanın
   güvenlik düzeltmeleri (parola/seed/`set -e`) HENÜZ CANLIDA DEĞİL. Canlı `main`'i izlediği için
   düzeltmeler ancak birleştirmeyle iner. **Bu birleştirmeden önce API suite'i yeşil görülmeli.**
2. **`APP_KEY` doğrulaması** — yukarıdaki "SENDE OLAN TEK KRİTİK İŞ" bölümü. Coolify UI işi.
3. **Coolify ortam değişkenlerinin denetimi.** `docker-compose.prod.yml` birçok anahtarda GÜVENLİ ama
   ÖZELLİKSİZ varsayılana düşüyor; Coolify'da tanımlı değillerse şu özellikler canlıda SESSİZCE kapalıdır:
   - `MAIL_MAILER` (varsayılan `log`) → **parola sıfırlama e-postası hiç kimseye gitmiyor olabilir**
   - `GEOCODING_DRIVER` (varsayılan `null`) → "Adresten Konum Al" çalışmıyor olabilir
