...
> 5. **Ölçüm:** mobil **1208/1208** · `flutter analyze` temiz. **CİHAZDA DOĞRULAMA YAPILMADI.**
>
...
> - **İki araç tuzağı yeniden ölçüldü:** `testWidgets` içinde `db.close()` sahte zamanda ASILIR
>   (saf sorgu testi `test()` olmalı); durdurulan `flutter test` yetim `flutter_tester` bırakıp
>   `sqlite3.dll`'i kilitler — DLL'i silme, süreci kapat.
